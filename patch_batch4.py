"""
Wire the batch 4 images into four monitoring pages.

Run from the repo root:

    python patch_batch4.py

Unzip batch4-images.zip into site/assets/img/ first.
Writes build/svc_monitoring.py.bak4.
"""
...
SRC = os.path.join(ROOT, "build", "svc_monitoring.py")
...
out = "\n".join(lines)
open(SRC + ".bak4", "wb").write(raw.encode("utf-8"))
open(SRC, "wb").write((out.replace("\n", "\r\n") if crlf else out).encode("utf-8"))

print("Patched:")
print("\n".join(report))
print("\nBackup at build/svc_monitoring.py.bak4")
